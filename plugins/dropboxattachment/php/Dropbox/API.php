...
class API
{
	...
	*/
	...
	/**
	 * @return OAuth\Consumer|OAuth\Consumer\ConsumerAbstract|null
	 */
	...
	public function putFile($file, $filename = false, $path = '', $overwrite = true)
	{
		if(file_exists($file)){
			if(filesize($file) <= 157286400){
				...
				$filename = (is_string($filename)) ? $filename : mb_basename($file);
				$params = array(
					'filename' => $filename,
	...
	public function getFile($file, $outFile = false, $revision = null)
	{
		...
		return array(
			'name' => ($outFile) ? $outFile : mb_basename($file),

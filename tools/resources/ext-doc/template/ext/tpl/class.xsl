    ...
    <xsl:template match="/docClass">        
        <div class="body-wrap">
            ...
            <table cellspacing="0">
				...
				<xsl:if test="xtypeName!=''">
					<tr>
						<td class="label">Xtype:</td>
						<td class="hd-info">
                        <xsl:choose>
                            <xsl:when test="xtypeName!=''"><xsl:value-of select="xtypeName"/></xsl:when>
                            <xsl:otherwise>None</xsl:otherwise>
                        </xsl:choose>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="ptypeName!=''">
					<tr>
						<td class="label">Ptype:</td>
						<td class="hd-info">
                        <xsl:choose>
                            <xsl:when test="ptypeName!=''"><xsl:value-of select="ptypeName"/></xsl:when>
                            <xsl:otherwise>None</xsl:otherwise>
                        </xsl:choose>
						</td>
					</tr>
            ...
            <a id="{className}-privateProps"> </a>
            <h2>Private Properties</h2>
            <xsl:choose>
                <xsl:when test="privateProperties">
                    <table cellspacing="0" class="member-table">
                        <tbody>
                            <tr>
                                <th colspan="2" class="sig-header">Property</th>
                            ...
                            <xsl:for-each select="privateProperties">
                                <xsl:variable name="inherited">
                                    <xsl:if test="/docClass/className!=className">inherited</xsl:if>
                                </xsl:variable>
                                <xsl:variable name="cls">
                ...
                <xsl:otherwise><div class="no-members">This class has no private properties.</div></xsl:otherwise>
            </xsl:choose>

            <a id="{className}-protectedProps"> </a>
            <h2>Protected Properties</h2>
            <xsl:choose>
                <xsl:when test="protectedProperties">
                    <table cellspacing="0" class="member-table">
                        <tbody>
                            <tr>
                                <th colspan="2" class="sig-header">Property</th>
                            ...
                            <xsl:for-each select="protectedProperties">
                                <xsl:variable name="inherited">
                                    <xsl:if test="/docClass/className!=className">inherited</xsl:if>
                                </xsl:variable>
                                <xsl:variable name="cls">
                ...
                <xsl:otherwise><div class="no-members">This class has no protected properties.</div></xsl:otherwise>
            </xsl:choose>
            ...
            <!--Private Methods-->
			<!--xsl:if test="privateMethods"-->
            <a id="{className}-privateMethods"> </a>
            <h2>Private Methods</h2>
            <xsl:choose>
                <xsl:when test="privateMethods">
                    <table cellspacing="0" class="member-table">
                        <tbody>
                            <tr>
                                <th colspan="2" class="sig-header">Method</th>
                            ...
                            <xsl:for-each select="privateMethods">
                                <xsl:variable name="inherited">
                                    <xsl:if test="/docClass/className!=className">inherited</xsl:if>
                                </xsl:variable>
                                <tr class="method-row expandable {$inherited}">
                ...
                <xsl:otherwise><div class="no-members">This class has no private methods.</div></xsl:otherwise>
            </xsl:choose>
			<!--/xsl:if-->

            <a id="{className}-protectedMethods"> </a>
            <h2>Protected Methods</h2>
            <xsl:choose>
                <xsl:when test="protectedMethods">
                    <table cellspacing="0" class="member-table">
                        <tbody>
                            <tr>
                                <th colspan="2" class="sig-header">Method</th>
                            ...
                            <xsl:for-each select="protectedMethods">
                                <xsl:variable name="inherited">
                                    <xsl:if test="/docClass/className!=className">inherited</xsl:if>
                                </xsl:variable>
                                <tr class="method-row expandable {$inherited}">
                ...
                <xsl:otherwise><div class="no-members">This class has no protected methods.</div></xsl:otherwise>
            </xsl:choose>			
            ...
            <!--Insertion points-->
            <a id="{className}-inserts"> </a>
            <h2>Insertion points</h2>
            <xsl:choose>
                <xsl:when test="inserts">
                  <table cellspacing="0" class="member-table">
                      <tbody>
                          <tr>
                              <th colspan="2" class="sig-header">Insertion point</th>
                              <th class="msource-header">Defined By</th>
                          </tr>
                          <xsl:for-each select="inserts">
                              <xsl:variable name="inherited">
                                <xsl:if test="/docClass/className!=className">inherited</xsl:if>
                              </xsl:variable>
                              <tr class="method-row expandable {$inherited}">
                                  ...
                                  <td class="sig">
                                      ...
                                      <div class="mdesc">
                                          ...
                                          <div class="long">
                                              <xsl:value-of select="description/longDescr" disable-output-escaping="yes"/>
                                              <div class="mdetail-params">
                                                  <strong style="font-weight: normal;">Component constructors will be called with the following arguments:</strong>
                                                  <ul>
                                                      <xsl:if test="count(params)=0">
                                                          <li>None.</li>
                                                      </xsl:if>
                ...
                <xsl:otherwise><div class="no-members">This class has no insertion points.</div></xsl:otherwise>
            </xsl:choose>

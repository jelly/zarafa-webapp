<?php
	require('server/exceptions/class.DispatcherException.php');
	...
	*/
	class Dispatcher
	{
		...
		function loadModule($moduleName, $id, $data)
		{
			...
			$path = BASE_PATH . 'server/modules/class.' . $moduleName . '.php';
		...
		function loadNotifier($notifierName)
		{
			...
			$path = BASE_PATH . 'server/notifiers/class.' . $notifierName . '.php';
			if (is_file($path) === true) {
				require_once($path);
